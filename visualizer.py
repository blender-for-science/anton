...
class Anton_OT_Visualizer(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if os.path.isfile(density_file):
            self.marchthecubes(inp_path=density_file, output_path=stl_file, density_thresh=scene.anton.density_out)

            bpy.ops.import_mesh.stl(filepath=stl_file, global_scale=10/scene.anton.res)
    ...
    def marchthecubes(inp_path, output_path, resolution=100, density_thresh=0.1):
        ...
        with open(inp_path, 'r') as f:
            ...
            while(line):
                ...
                if section_match:
                    base_coord = np.array(list(map(int, [section_match.group('X'), section_match.group('Y'), section_match.group('Z')])))
        ...
        pts = (np.array(verts, dtype=np.int) - 8)/resolution - 0.5
        lower_bound = np.floor(np.min(pts, axis=0))
        upper_bound = np.ceil(np.max(pts, axis=0))

        grid, _, _ = np.mgrid[lower_bound[0]:upper_bound[0]:1/resolution, 
                                lower_bound[1]:upper_bound[1]:1/resolution, 
                                lower_bound[2]:upper_bound[2]:1/resolution]

        data_indices = np.array(np.ceil((pts - lower_bound)/(1/resolution)), dtype=np.int)
        data = 0.0 * grid
        # mask??
